...
namespace CodeScales.Http.Network
{
  internal class HttpConnection
  {
    ...
    internal void Connect()
    {
      ...
      m_socket.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.SendTimeout, m_timeout);
      m_socket.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.ReceiveTimeout, m_timeout);
    }
    ...
    public HttpResponse ReceiveResponseHeaders()
    {
      ...
      try {
        while (this.m_socket.Receive(bytes, 0, 1, SocketFlags.None) > 0) {
          AddByte(byteBuffer, bytes[0], lastBytes, ref lastIndex);
          if (bytes[0] == '\n' && LastIsHeaderFinish(lastIndex, lastBytes))
            break;
        }
      } catch (Exception ex) {
        Console.WriteLine("ReceiveResponseHeaders header:{0}", Encoding.ASCII.GetString(byteBuffer.ToArray()));
        throw ex;
      }
      string Header = Encoding.ASCII.GetString(byteBuffer.ToArray());
    ...
    private string ReceiveLine()
    {
      ...
      try {
        while (this.m_socket.Receive(bytes, 0, 1, SocketFlags.None) > 0) {
          AddByte(byteBuffer, bytes[0], lastBytes, ref lastIndex);
          if (bytes[0] == '\n' && LastIsLineFinish(lastIndex, lastBytes))
            break;
        }
      } catch (Exception ex) {
        Console.WriteLine("ReceiveLine line:{0}", Encoding.ASCII.GetString(byteBuffer.ToArray()));
        throw ex;
      }
      string line = Encoding.ASCII.GetString(byteBuffer.ToArray());
      return line.Replace("\r\n", "");
    }
    ...
    private static void AddByte(List<byte> buffer, byte c, byte[] lastBytes, ref int lastIndex)
    {
      lastBytes[lastIndex % lastBytes.Length] = c;
      ++lastIndex;
      buffer.Add(c);
    }

    private static bool LastIsHeaderFinish(int lastIndex, byte[] lastBytes)
    {
      bool ret=false;
      if (lastIndex >= 4) {
        if (lastBytes[(lastIndex - 4) % 4] == '\r' && lastBytes[(lastIndex - 3) % 4] == '\n' && lastBytes[(lastIndex - 2) % 4] == '\r' && lastBytes[(lastIndex - 1) % 4] == '\n')
          ret = true;
      }
      return ret;
    }
    private static bool LastIsLineFinish(int lastIndex, byte[] lastBytes)
    {
      bool ret = false;
      if (lastIndex >= 2) {
        if (lastBytes[(lastIndex - 2) % 4] == '\r' && lastBytes[(lastIndex - 1) % 4] == '\n')
          ret = true;
      }
      return ret;
    }

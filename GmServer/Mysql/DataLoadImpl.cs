...
namespace GmServer
{
  internal static class DataLoadImpl
  {
    ...
    /// static
    ///==============================================================================================
    internal static void LoadSingleRowWithCallback(Type dsType, string primaryKey, MyAction<IMessage> resultCallback)
    {
      IMessage dataMsg = LoadSingleRow(dsType, primaryKey);
      if (null != resultCallback) {
        DataOperator.Instance.QueueAction(resultCallback, dataMsg);
      }
    }
    internal static void LoadMultiRowsWithCallback(Type dsType, string foreignKey, MyAction<List<IMessage>> resultCallback)
    {
      List<IMessage> datas = LoadMultiRows(dsType, foreignKey);
      if (null != datas) {
        DataOperator.Instance.QueueAction(resultCallback, datas);
      }
    }
    internal static void LoadTableWithCallback(Type dsType, MyAction<List<IMessage>> resultCallback)
    {
    ...
    internal static IMessage LoadSingleRow(Type dsType, string primaryKey)
    {
      ...
      MessageDescriptor md = (MessageDescriptor)dsType.InvokeMember("Descriptor", 
        BindingFlags.Static | BindingFlags.Public | BindingFlags.GetProperty, null, null, null);
    ...
    internal static List<IMessage> LoadMultiRows(Type dsType, string foreignKey)
    {
      ...
      string statement = string.Format(
        "select * from {0} where {1} = '{2}' and IsValid = 1", tableName, foreignKeyName, foreignKey);
    ...
    private static List<IMessage> ExecuteLoadSQL(Type tableType, string tableName, string statement, MessageDescriptor md)
    {
      ...
      IBuilder builder = (IBuilder)tableType.InvokeMember("CreateBuilder",
        ...
        lock (s_Guard) {
          if (!s_TableColumnDict.TryGetValue(tableName, out columnInfos)) {
            string sql = string.Format("select column_name,data_type,column_type from information_schema.columns where table_name='{0}' and table_schema='{1}'",
              tableName, GmServerConfig.DataBase);

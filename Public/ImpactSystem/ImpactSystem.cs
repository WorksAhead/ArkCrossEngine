...
namespace ArkCrossEngine
{
    ...
    public sealed class ImpactSystem
    {
        ...
        public static ImpactEventHandler EventStopAllImpact;
        ...
        {
            //LogSystem.Debug("SendImpactToCharacter impactId = {0} skillId = {1}", impactId, skillId);
            ...
            if (ret)
            {
                if (null != sender)
                {
                    ...
                    if (null != target)
                    {
                        ...
                        //LogSystem.Debug("SendImpactToCharacter impactId = {0} skillId = {1}, scrPos = ({2},{3},{4}), scrDir = {5}, tarPos = ({6},{7},{8}})", impactId, skillId, srcPos.X, srcPos.Y, srcPos.Z, srcDir, tarPos.X,
                        //  tarPos.Y, tarPos.Z);
        ...
        public void StopAllImpact(CharacterInfo target)
        {
            List<ImpactInfo> impactInfos = target.GetSkillStateInfo().GetAllImpact();
            for (int i = 0; i < impactInfos.Count; ++i)
            {
                StopImpactImpl(target, impactInfos[i].m_ImpactId);
            }
            if (null != EventStopAllImpact)
            {
                EventStopAllImpact(null, target.GetId(), -1);
            }
        }

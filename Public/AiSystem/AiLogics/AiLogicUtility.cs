...
namespace ArkCrossEngine
{
    ...
    public sealed class AiLogicUtility
    {
        ...
        public static CharacterInfo GetNearstTargetHelper(CharacterInfo srcObj, CharacterRelation relation, AiTargetType type)
        {
            ...
            ISpatialSystem spatialSys = srcObj.SpatialSystem;
            if (null != spatialSys)
            {
        ...
        {
            return GetRandomTargetHelper(srcObj, relation, AiTargetType.ALL);
        }
        public static CharacterInfo GetRandomTargetHelper(CharacterInfo srcObj, CharacterRelation relation, AiTargetType type)
        {
            ...
            if (null != spatialSys)
            {
                List<ISpaceObject> objs = spatialSys.GetObjectInCircle(srcObj.GetMovementStateInfo().GetPosition3D(), srcObj.ViewRange, (distSqr, obj) => IsWantedTargetForRandomTarget(srcObj, relation, type, obj));
                int index = CrossEngineHelper.Random.Next(objs.Count);
        ...
        private static void GetInterestestTarget(CharacterInfo srcObj, CharacterRelation relation, AiTargetType type, float powDist, ISpaceObject obj, ref float minPowDist, ref CharacterInfo interestestTarget)
        {
            ...
            if (null != target && !target.IsDead())
            {
                ...
                if (null != npcTarget && !npcTarget.IsCombatNpc())
                {
                    return;
                }
        ...
        public static void CommonIdleHandler(UserInfo user, AiCommandDispatcher aiCmdDispatcher, long deltaTime, AbstractUserStateLogic logic)
        {
            ...
            {
                info.Time = 0;
                CharacterInfo target = AiLogicUtility.GetRandomTargetHelper(user, CharacterRelation.RELATION_ENEMY, AiTargetType.ALL);
                if (null != target)
                {
                    info.Target = target.GetId();
                    user.GetMovementStateInfo().IsMoving = false;
        ...
        public static void CommonPatrolHandler(NpcInfo npc, AiCommandDispatcher aiCmdDispatcher, long deltaTime, AbstractNpcStateLogic logic)
        {
            ...
            if (info.Time > 100)
            {
                ...
                info.Time = 0;
                CharacterInfo target = AiLogicUtility.GetRandomTargetHelper(npc, CharacterRelation.RELATION_ENEMY, AiTargetType.USER);
                if (null != target)
                {
                    info.Target = target.GetId();
                    logic.NotifyNpcTargetChange(npc);
            ...
            if (null != data)
            {
                ...
                {//沿路点列表移动的逻辑
                    ...
                    {//向指定路点移动（避让移动过程）
...
#endif
                        if (Geometry.DistanceSquare(newVelocity, new Vector3()) <= speedSquare * 0.25f)
                        {//避让计算的移动速度变小（说明没有更好的保持原速的选择，停止）
                            npc.GetMovementStateInfo().IsMoving = false;
                            logic.NotifyNpcMove(npc);
                            data.IsUsingAvoidanceVelocity = false;
                        }
                        else if (findObstacle)
                        {//当前移动方向遇到阻挡，停止移动，触发寻路
                            npc.GetMovementStateInfo().IsMoving = false;
                            logic.NotifyNpcMove(npc);
                            data.IsUsingAvoidanceVelocity = false;
                        }
                        else if (data.UpdateTime > 1000)
                        {//避让速度改变每秒一次（表现上更像人类一些）
                            data.UpdateTime = 0;
                        ...
                        else if (Geometry.DistanceSquare(velocity, newVelocity) > 9.0f)
                        {//没有到速度改变周期，但避让方向需要大幅调整
                            if (Geometry.Dot(newVelocity, prefVelocity) > 0)
                            {//如果是调整为与目标方向一致，则进行调整
                                float newAngle = Geometry.GetYAngle(new Vector2(0, 0), new Vector2(newVelocity.X, newVelocity.Z));
                                npc.GetMovementStateInfo().SetMoveDir(newAngle);
                                if (faceIsMoveFir)
                        ...
                        else if (!npc.GetMovementStateInfo().IsMoving && velocity.LengthSquared() > speedSquare * 0.25f)
                        {//正常移动过程，继续移动
                            velocity.Normalize();

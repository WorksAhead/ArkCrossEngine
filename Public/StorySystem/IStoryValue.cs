...
namespace StorySystem
{
    ...
    {
        // 从DSL语言初始化值实例
        void InitFromDsl(ScriptableData.ISyntaxComponent param);
        // 克隆一个新实例，每个值只从DSL语言初始化一次，之后的实例由克隆产生，提升性能
        IStoryValue<T> Clone();
        // 从引用的参数获取参数值
        void Evaluate(object iterator, object[] args);
        // 从引用的变量获取变量值
        void Evaluate(StoryInstance instance);
        // 语义分析，配合上下文instance进行语义分析，在执行前收集必要的信息
        void Analyze(StoryInstance instance);
        // 是否已经有值，对常量初始化后即产生值，对参数、变量与函数则在Evaluate后产生值
        bool HaveValue { get; }
        // 具体的值
        T Value { get; }
    }

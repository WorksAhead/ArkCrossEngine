...
namespace StorySystem.CommonValues
{
    ...
    internal sealed class RandomIntValue : IStoryValue<object>
    {
        ...
        private void TryUpdateValue(StoryInstance instance)
        {
            if (m_Min.HaveValue && m_Max.HaveValue)
            {
                ...
                int min = m_Min.Value;
                int max = m_Max.Value;
                m_Value = CrossEngineHelper.Random.Next(min, max);
            }
        }
    ...
    internal sealed class RandomFloatValue : IStoryValue<object>
    {
        ...
        {
            m_HaveValue = true;
            m_Value = CrossEngineHelper.Random.NextFloat();
        }
    ...
    internal sealed class Vector3DistanceValue : IStoryValue<object>
    {
        ...
        private void TryUpdateValue()
        {
            ...
            {
                m_HaveValue = true;
                m_Value = Geometry.Distance(m_Pt1.Value, m_Pt2.Value);
            }
        }
    ...
    internal sealed class Vector2ListValue : IStoryValue<object>
    {
        ...
        private void TryUpdateValue()
        {
            ...
            {
                m_HaveValue = true;
                List<Vector2> list = Converter.ConvertVector2DList(m_ListString.Value);
                m_Value = new List<object>();
                for (int i = 0; i < list.Count; ++i)
                {
                    m_Value.Add(list[i]);
    ...
    internal sealed class Vector3ListValue : IStoryValue<object>
    {
        ...
        private void TryUpdateValue()
        {
            ...
            {
                m_HaveValue = true;
                List<Vector3> list = Converter.ConvertVector3DList(m_ListString.Value);
                m_Value = new List<object>();
                for (int i = 0; i < list.Count; ++i)
                {
                    m_Value.Add(list[i]);
    ...
    internal sealed class RandomFromListValue : IStoryValue<object>
    {
        ...
        private void TryUpdateValue()
        {
            if (m_ListValue.HaveValue)
            {
                ...
                int ix = CrossEngineHelper.Random.Next(ct);
                if (ix >= 0 && ix < ct)
                {
